...
describe("useTodoListフックのテスト", () => {
  ...
  test("addTodoが実行されると、todoListとlocalStorageが更新されること", () => {
    const { result } = renderHook(() => useTodoList())
    act(() => {
      result.current.addTodo("洗濯", "浅野太郎", "2024-10-27")
    })
    ...
    expect(newTodo.deadline).toBe("2024-10-27")
    ...
    expect(todoList[2].deadline).toBe("2024-10-27")
  })
  ...
  test("deleteTodoが実行されると、todoListとlocalStorageが更新されること", () => {
    ...
    expect(todo.deadline).toBe("2024-10-28")
    ...
    expect(todoList[0].deadline).toBe("2024-10-28")
  })
